		...
		if (EVClient.shouldRender()) {
			...
			if (!(mc.screen instanceof DeathScreen)) {
				...
				Matrix4f matrix4f = Matrix4f.orthographic(
						0.0F,
						(float) (mc.getWindow().getWidth() / mc.getWindow()
						                                       .getGuiScale()),
						0.0F,
						(float) (mc.getWindow().getHeight() / mc
				...
				renderVisuals(
						stack,
						VisualManager.visuals(VisualCategory.overlay),
						manager,
						screenWidth,
						screenHeight,
						partialTicks
				...
				renderVisuals(
						stack,
						VisualManager.visuals(VisualCategory.particle),
						manager,
						screenWidth,
						screenHeight,
						partialTicks
				...
				renderVisuals(
						stack,
						VisualManager.visuals(VisualCategory.shader),
						manager,
						screenWidth,
						screenHeight,
						partialTicks
				...
				if (EnhancedVisuals.MESSAGES.enabled) {
					...
					if (lastRenderedMessage != null)
						mc.font.drawShadow(
								new PoseStack(),
								"\"" + lastRenderedMessage + "\"",
								mc.screen.width / 2 - mc.font.width(lastRenderedMessage) / 2,
								114,
								16777215
						);
				}
