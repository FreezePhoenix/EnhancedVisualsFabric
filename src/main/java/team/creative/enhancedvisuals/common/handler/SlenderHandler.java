...
public class SlenderHandler extends VisualHandler {
	...
	public void tick(@Nullable Player player) {
		...
		if (player != null) {
			...
			Entity mob = player.level.getNearestEntity(
					EnderMan.class,
			...
			if (mutantEnderman != null)
				mob = player.level.getNearestEntity(
						mutantEnderman,
						TargetingConditions.forNonCombat(),
						player,
						d0,
			...
			if (mob != null) {
				float distModifier = (float) (1.0F / Math.pow(Math.sqrt(Math.pow(
						d0 - mob.getX(),
						2
				) + Math.pow(d1 - mob.getY(), 2) + Math.pow(d2 - mob.getZ(), 2)) / 3.0D, 2));
			...
			intensity = slenderVisual.getOpacity() / 2;
		}
